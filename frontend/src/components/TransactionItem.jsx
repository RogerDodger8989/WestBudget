...
const TransactionItem = ({ data, onClick, onEditNote, isSelected = false, onToggleSelect }) => {
  ...
  return (
    <div className={`grid grid-cols-12 gap-4 px-6 py-4 border-b border-zinc-100 dark:border-zinc-800/50 hover:bg-zinc-50 dark:hover:bg-zinc-800/40 transition-colors group items-center ${
      ...
      <div onClick={onClick} className={`font-medium text-zinc-900 dark:text-zinc-200 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors truncate cursor-pointer ${onToggleSelect ? 'col-span-2' : 'col-span-3'}`}>
        {title}
      </div>
      <div onClick={onClick} className="col-span-2 text-sm text-zinc-500 cursor-pointer">{date}</div>
      <div onClick={onClick} className="col-span-2 text-sm cursor-pointer">
      ...
      <div onClick={onClick} className="col-span-1 text-center cursor-pointer">
      ...
      <div onClick={onClick} className="col-span-1 flex justify-center cursor-pointer">
